...
class HomeAwayTemperature(){
//object HomeAwayTemperature extends App {
    var maxTemp:Float = 0
    var minTemp:Float = 0
...
def homeAwayScreen(): Unit ={
    while(actionoption != "3") {
        println(s"${RESET}${BLUE}            HOME/AWAY TEMPERATURE${RESET}\n")
        ...
        actionoption match {
            case "1" => listAllStats()
            case "2" => tempStats()
            case "3" => println("Exiting Home/Away Temperature")
    ...
    def listAllStats():Unit={
        try {
            val dfAllStats = spark.read.json("hdfs://localhost:9000/user/analysethatJSON/temperature/")
            dfAllStats.orderBy("TypeStat").show(dfAllStats.count.toInt, false)
        }catch{
            case e: Throwable => e.printStackTrace
                println("Error listing temperature JSON files.")
        }
    ...
    def tempStats():Unit={
        ...
        val vtot = total(optionInit,optionFinal,optionMaxTemp,optionMinTemp)
            if(vtot > 0 ){
                ...
                val vhome = (home(optionInit,optionFinal,optionMaxTemp,optionMinTemp)/vtot)*100
                println("Calculating away stats....")
                val vaway = (away(optionInit,optionFinal,optionMaxTemp,optionMinTemp)/vtot)*100
                println("Calculating draw stats....")
                val vdraw = (draw(optionInit,optionFinal,optionMaxTemp,optionMinTemp)/vtot)*100
                val vtypeS = "TEMP"+ optionInit.toString + optionFinal.toString + optionMaxTemp.toString + optionMinTemp.toString
                println("Saving JSON....")
                val df = sc.parallelize(Seq((vtypeS,vtot,vhome,vaway,vdraw,maxTemp,minTemp))).toDF("TypeStat", "Total Games", "Home", "Away", "Draw", "MaxTemp", "MinTemp")
                df.write.mode(saveMode = "Append").json("hdfs://localhost:9000/user/analysethatJSON/temperature/")
    ...
    def draw(initYear: Int, finalYear: Int, pmaxTemp: Float, pminTemp: Float): Float={
        //home winnings
        val homeWiningsPeriod = spark.sql("Select count(*) " +
          "from "+ table +
          " where schedule_season between "+ initYear.toString + " and " + finalYear.toString +
